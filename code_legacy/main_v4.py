...
def sb_heartbeat_silent(connected=True):
    """PATCH direct sur la table — aucun log, pas de boucle infinie."""
    if not _wifi_ok:
        return
    try:
        r = urequests.patch(
            SUPABASE_URL + "/rest/v1/alarm_device_status?id=eq.1",
            headers=_headers(),
            data=ujson.dumps({
...
def main():
    ...
    while True:
        ...
        if global_etat == ETAT_ARMEE and pir.value() == 1:
            ...
            sb_log("info", "intrusion_detected",
